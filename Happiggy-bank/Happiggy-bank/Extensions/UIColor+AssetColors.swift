...
extension UIColor {
    ...
    static func highlight(color: NoteColor) -> UIColor {
        UIColor(named: AssetColor.noteHighlight.rawValue + color.capitalizedString) ?? .systemGray3
    }
    
    /// 애셋에 추가한 쪽지 색상을 반환하는 메서드
    static func note(color: NoteColor) -> UIColor {
        UIColor(named: AssetColor.note.rawValue + color.capitalizedString) ?? .white
    }
}
